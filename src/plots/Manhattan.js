...
export const Manhattan = ({ dyn, stat, threshold, onSNPClick, phenoId, selectedCohort, selectedStudy, filterLimit }) => {
    ...
    const chrPositions = useMemo(() => {
        ...
        for (let i = 0; i < CHR_COUNT; i++) {
            ...
            currentPos += chrFrac + (i < CHR_COUNT - 1 ? GAP * scaleFactor : 0);
        }
        return positions;
    }, []);
    ...
    const generateYAxisConfig = (maxValue) => {
        ...
        return { ticks, maxRange };
    };

    useEffect(() => {
        const allYValues = [...stat, ...dyn].flatMap(d => d.y);
        const maxYValue = Math.ceil(Math.max(...allYValues, threshold || 0));
        const maxLeadSNPLog10p = leadSNPs.reduce((max, snp) => Math.max(max, snp.lead_snp?.log10p || 0), 0);
        const absoluteMaxY = Math.max(maxYValue, maxLeadSNPLog10p);
        const { ticks, maxRange } = generateYAxisConfig(absoluteMaxY);
        const xAxisTicks = calculateChromosomePositions();
        ...
        const getImagePath = async () => {
            try {
                const study = selectedStudy === 'mrmega' ? 'mrmega' : 'gwama';
                const response = await fetch(
                    `/api/getManhattanPlot?phenoId=${phenoId}&cohortId=${selectedCohort}&study=${study}&plotType=manhattan`
                );
                if (!response.ok) throw new Error('Failed to fetch Manhattan plot');
                const blob = await response.blob();
                return URL.createObjectURL(blob);
            } catch (error) {
                console.error('Error fetching Manhattan plot:', error);
        ...
        (async () => {
            ...
            setLayout({
                ...
                xaxis: {
                    ...
                    tickmode: 'array',
                    tickvals: xAxisTicks,
                    ticktext: Array.from({ length: CHR_COUNT }, (_, i) => `${i + 1}`),
                    showgrid: false,
                    zeroline: false,
                    showline: true,
                    linewidth: 1,
        ...
        return () => {
            if (layout?.images?.[0]?.source?.startsWith('blob:')) {
                URL.revokeObjectURL(layout.images[0].source);
            }
        };
    }, [stat, dyn, threshold, leadSNPs, phenoId, selectedCohort, selectedStudy]);

    const prepareRegularData = useMemo(() => {
        const traces = [];
        const allData = [...stat, ...dyn];

        for (let chrIndex = 0; chrIndex < CHR_COUNT; chrIndex++) {
            const chrData = allData.filter(d => d.chr === chrIndex + 1);
    ...
    const prepareLeadSNPData = useMemo(() => {
        if (!leadSNPs.length || filterLimit === 'None') return null;

        const sortedSNPs = [...leadSNPs].sort((a, b) =>
            parseFloat(b.lead_snp.log10p) - parseFloat(a.lead_snp.log10p)
        );
        const filteredSNPs = filterLimit === 'all' ? sortedSNPs : sortedSNPs.slice(0, parseInt(filterLimit));
    ...
    }, [leadSNPs, filterLimit, selectedCohort]);
    ...
    const allData = useMemo(() => {
        const regularData = prepareRegularData;
        const leadSNPTrace = prepareLeadSNPData;
        return leadSNPTrace ? [...regularData, leadSNPTrace] : regularData;
    }, [prepareRegularData, prepareLeadSNPData]);
